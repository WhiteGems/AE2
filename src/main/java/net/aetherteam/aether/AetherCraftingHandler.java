    ...
    implements ICraftingHandler
{
    public void onCrafting(EntityPlayer player, ItemStack stack, IInventory craftMatrix)
    {
        if (((stack.itemID == AetherBlocks.Altar.blockID) ||
                (!isGravititeTool(stack.itemID))) || (
                    (stack.itemID == Item.silk.itemID) && (craftMatrix != null)))
            for (int i = 0; i < craftMatrix.getSizeInventory(); i++)
            {
                ItemStack stackin = craftMatrix.getStackInSlot(i);

                if ((stackin != null) && (stackin.itemID == Item.shears.itemID) &&
                        (stackin.getItemDamage() + 6 < stackin.getMaxDamage()))
                {
