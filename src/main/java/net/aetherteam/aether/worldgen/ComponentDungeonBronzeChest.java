    ...
    public boolean addComponentParts(World world, Random rand, StructureBoundingBox ChunkBox)
    {
        ...
        for (int chestCount = 1; chestCount <= 4; chestCount++)
        {
            ...
            else if ((ChunkBox.isVecInside(i, j, k)) && (world.getBlockId(i, j, k) != AetherBlocks.SkyrootChestMimic.blockID) && (rand.nextInt(3) == 0))
            {
                ...
                Dungeon dungeon = DungeonHandler.instance().getInstanceAt(i, j, k);
                dungeon.registerSafeBlock(i, j, k, AetherBlocks.SkyrootChestMimic.blockID, chestDirection);
            }
        }

        placeBlockAtCurrentPosition(world, Block.mobSpawner.blockID, 0, x, y, z, ChunkBox);
        TileEntityMobSpawner spawner = (TileEntityMobSpawner)world.getBlockTileEntity(x, y, z);

        if (spawner != null)
        {
            String mobID = rand.nextInt(3) == 1 ? "SentryGolem" : "Sentry";
            spawner.func_98049_a().setMobID(mobID);
            Dungeon dungeon = DungeonHandler.instance().getInstanceAt(x, y, z);
            dungeon.registerMobSpawner(x, y, z, mobID);
        }
