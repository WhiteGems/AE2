...
public class ComponentDungeonBronzeHost extends ComponentDungeonBronzeRoom
{
    ...
    public boolean addComponentParts(World par1World, Random par2Random, StructureBoundingBox ChunkBox)
    {
        ...
        if (ChunkBox.isVecInside(x + 1, y, z + 1))
        {
            ...
            host.setPosition((double)(x + 1), (double)y, (double)(z + 1));
            DungeonHandler handler = DungeonHandler.instance();
            Dungeon dungeon = handler.getInstanceAt(MathHelper.floor_double((double)(x + 1)), MathHelper.floor_double((double)y), MathHelper.floor_double((double)(z + 1)));

            if (dungeon != null)
            {
                dungeon.registerEntity((float)(x + 1), (float)y, (float)(z + 1), host);
            }
        }
