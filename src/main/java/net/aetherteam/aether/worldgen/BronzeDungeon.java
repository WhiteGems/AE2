...
public class BronzeDungeon extends MapGenBase
    implements Serializable
{
    ...
    protected void recursiveGenerate(World par1World, int par2, int par3, int par4, int par5, byte[] par6ArrayOfByte)
    {
        if (!this.structureMap.containsKey(Long.valueOf(ChunkCoordIntPair.chunkXZ2Int(par2, par3))))
        {
            ...
            {
                StructureBronzeDungeonStart var7 = getStructureStart(par2, par3);
    ...
    public boolean generateStructuresInChunk(World par1World, Random par2Random, int par3, int par4, double[] dirtDeapth)
    {
        ...
        try
        {
            while (i.hasNext())
            {
                StructureBronzeDungeonStart var9 = (StructureBronzeDungeonStart)i.next();

                if (var9.getBoundingBox().intersectsWith(var5 - 15, var6 - 15, var5 + 31, var6 + 31))
                {
                    var9.generateStructure(par1World, par2Random, new StructureBoundingBox(var5, var6, var5 + 15, var6 + 15), dirtDeapth);
                    var7 = true;
                }
            }
        }
        catch (ConcurrentModificationException e)
        {
            ...
            if (this.tried < 4)
            {
                generateStructuresInChunk(par1World, par2Random, par3, par4, dirtDeapth);
            }
        }
        ...
        while (var4.hasNext())
        {
            ...
            if (dungeonStart.getBoundingBox().intersectsWith(x, z, x, z))
            {
                ...
                while (var6.hasNext())
                {
                    ...
                    if (intersectsWith(room.getBoundingBox(), x, y, z))
                    {
